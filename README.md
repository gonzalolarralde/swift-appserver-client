...
Release `0.144.5` is generated from the experimental app-server schema exposed
by the stable public Codex tag `rust-v0.144.5`. App-server experimental APIs can
change between Codex releases, so matching these versions is recommended.

## Requirements

- Swift 6.2 or newer.
- Codex `0.144.5` must be available on `PATH` for the stdio connection and smoke
  executable.
...
- The generated `multiAgentMode` fields are retained for wire compatibility,
  but Codex `0.144.5` ignores them and derives proactive delegation from Ultra
  reasoning effort.
